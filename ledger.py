...
from sqlalchemy.sql import text as sql_text  # alias to avoid name clashes with variables
...
# ==============================
# Local mic SpeechRecognition STT
# ==============================
def stt_local(language_code: str = "en") -> str:
    """
    SpeechRecognition-based local STT.
    language_code: "en" (English), "hi" (Hindi), or "en-IN"/"hi-IN".
    Requires: pip install SpeechRecognition pyaudio
    """
    try:
        import speech_recognition as sr
    except Exception as e:
        raise RuntimeError("SpeechRecognition is not installed. Install with: pip install SpeechRecognition") from e

    rec = sr.Recognizer()
    try:
        with sr.Microphone() as src:
            st.info("Listening... please speak (up to 10 seconds).")
            rec.adjust_for_ambient_noise(src, duration=0.5)
            audio = rec.listen(src, timeout=10, phrase_time_limit=10)
        text_val = rec.recognize_google(audio, language=language_code)
        return text_val
    except sr.WaitTimeoutError:
        raise RuntimeError("No speech detected within timeout. Try again.")
    except sr.UnknownValueError:
        raise RuntimeError("Could not understand the audio. Please try again.")
    except sr.RequestError as e:
        raise RuntimeError(f"Speech service request failed: {e}")
    except Exception as e:
        raise RuntimeError(f"Local mic error: {e}")

# ==============================
# Voice Ledger UI
# ==============================
st.subheader("🎤 Record with local microphone")

lang_choice = st.selectbox("Language", ["Auto English (en)", "Auto Hindi (hi)"], index=0)
lang_code = "en" if "English" in lang_choice else "hi"
...
with col1:
    if st.button("🎙️ Speak Now"):
        try:
            spoken_text = stt_local(lang_code)
            st.success(f"Transcribed: {spoken_text}")
            parsed = parse_text_transaction(spoken_text, 0.9)
            st.session_state.parsed = parsed
        except Exception as e:
            st.error(str(e))
with col2:
    st.caption("Tip: keep it short (5–10s) and speak clearly.")
